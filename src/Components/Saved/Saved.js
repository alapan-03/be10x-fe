...
function Saved() {
  ...
  const currentItems = savedFunds?.length>0 && savedFunds.slice(
    startIndex,
  ...
  return (
    <div className="app">
      ...
      <h1 className="title">⭐ Saved Mutual Funds</h1>

      {savedFunds?.length === 0 ? (
        <p className="no-results">You haven't saved any funds yet.</p>
      ) : (
        <div className="card-container">
          {currentItems.length>0 && currentItems.map((fund) => (
            <div
              onClick={() => fetchDetails(fund.schemeCode)}
            >
              <MutualFundCard
                key={fund.schemeCode}
                fund={fund}
                isSaved={true}
                onToggleSave={() => {}} // Optional here
              />
            </div>
          ))}
        </div>
